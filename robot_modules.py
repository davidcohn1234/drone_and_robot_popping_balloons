...
class RobotModules:
    ...
    def drive_speed(self, forward_backward, left_right, yaw_speed, timeout):
        if forward_backward is None or left_right is None or yaw_speed is None:
            return
        print(f'self.ninja1.chassis.drive_speed({forward_backward}, {left_right}, {yaw_speed}, {timeout})')
        self.ninja1.chassis.drive_speed(x=forward_backward, y=left_right, z=yaw_speed, timeout=timeout)
