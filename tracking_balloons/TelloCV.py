...
import \
    threading  # Python library that allows you to create multiple threads to run multiple functions at the same time.
...
def main():
    ...
    global exiting
    tellotrack = TelloCV()
    threading.Thread(target=frame_save, args=[
        tellotrack]).start()  # Create a thread that runs the function frame_save while main runs in the current thread
...
class TelloCV(object):
    ...
    def toggle_recording(self, speed):
        ...
        if self.record:
            ...
            self.out_name = 'C:/Users/eliza/OneDrive/Computer_Files/Desktop/Desktop/Fourth_Year/SPRING_2019/PROCESS_CONTROLS/Untitled_Folder/telloCV-master/Images/tello-{}.mp4'.format(
                *datename)
            print("Outputting video to:", self.out_name)

            self.out_stream_writer = cv2.VideoWriter(self.out_name, cv2.CV_FOURCC('P', 'I', 'M', '1'), 25,
                                                     (self.vid_stream.width, self.vid_stream.height))
